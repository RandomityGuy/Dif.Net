    ...
    public class CSXExporter
    {
        ...
        public void WriteBrush(XElement brushes,ConvexHull hull)
        {
            ...
            var hullpointsdoublearray = interior.hullSurfaceIndices.Skip(hull.surfaceStart).Take(hull.surfaceCount).Select(c =>
            {
                ...
                {
                    return (c & 0x80000000) == 0x80000000 ? interior.surfaces[(int)(c & ~0x80000000)] : interior.surfaces[c];
                }
            ...
            foreach (var surface in surfaces)
            {
                ...
                // var indicestr = hullpoints.IndexOf(interior.points[indices[0]]).ToString() + " " + hullpoints.IndexOf(interior.points[indices[i + 1]]) + " " + hullpoints.IndexOf(interior.points[indices[i + 2]]);

                for (var i = 0; i < surface.windingCount - 2; i ++)
                {
                    ...
                    var indicestr = string.Format("{0} {1} {2}", index0, index1, index2);
